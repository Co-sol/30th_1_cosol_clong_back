...
class UserInfoSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        allowed_fields = {'name', 'clean_sense', 'clean_type', 'profile', 'evaluation_status'}
        for field in attrs.keys():
            if field not in allowed_fields:
                raise serializers.ValidationError(f"'{field}' 필드는 수정할 수 없습니다.")
    ...
    def validate_clean_sense(self, value):
        if value is not None and (value < 0 or value > 100):
            raise serializers.ValidationError("청소 민감도는 0~100 사이의 값이어야 합니다.")
        return value

    def update(self, instance, validated_data):
        updated = False

        if 'clean_type' in validated_data and validated_data['clean_type'] != instance.clean_type:
