...
urlpatterns = [
    ...
    path('<int:group_id>/evaluation-view/', GroupEvalAverageView.as_view(), name='group-eval-average'), 
    path('<int:group_id>/<int:review_id>/logs-feedback/', ChecklistFeedbackView.as_view(), name='feedback'),
    path('<int:group_id>/logs/', GroupLogView.as_view(), name='log-view'),
    path('<int:group_id>/logs-pending/', PendingReviewListView.as_view()),
]
